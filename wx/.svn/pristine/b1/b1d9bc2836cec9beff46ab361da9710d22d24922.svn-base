...
Page({
    ...
    setzch: function (ev) {
        ...
        me.setData(
            {
                tdpgs: ev.detail.value
            }
        )
    },
    bindOk: function (ev) {
        var sCmd = { "cmd": "user.Qualification", "data": { "rzlx": 2, "tdgjszch": this.data.tdpgs, "form_id": ev.detail.formId } };
        WxNotificationCenter.postNotificationName("send", sCmd);
    },
    getUserinfo: function (res) {
        switch (res.data.code) {
            case 0:
                this.setData({
                    files: res.data.data[0].td_img == "" ? 'https://www.yiqiyun.org/filepgb/file_img/td.jpg' : res.data.data[0].td_img,
                    tdpgs: res.data.data[0].tdgjszch,
